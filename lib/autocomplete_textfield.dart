...
class AutoCompleteTextFieldState<T> extends State<AutoCompleteTextField> {
                    ...
                    child: new Card(
                      ...
                      children: filteredSuggestions.map((suggestion) {
                        return new Row(children: [
                          new Expanded(
                              child: new InkWell(
                                  child: itemBuilder != null ? itemBuilder!(context, suggestion) : SizedBox.shrink(),
                                  onTap: () {
                                    setState(() {
  ...
  List<T> getSuggestions(List<T> suggestions, Comparator<T>? sorter, Filter<T>? filter, int maxAmount, String? query) {
    ...
    suggestions = suggestions.where((item) => (filter != null ? filter(item, query) : false)).toList();
...
class SimpleAutoCompleteTextField extends AutoCompleteTextField<String> {
  ...
  @override
  State<StatefulWidget> createState() => new AutoCompleteTextFieldState<String>(suggestions, textChanged, textSubmitted, onFocusChanged, itemSubmitted, (context, item) {
        return new Padding(padding: EdgeInsets.all(8.0), child: new Text(item));
      }, (a, b) {
        return a.compareTo(b);
      }, (item, query) {
        return item.toLowerCase().startsWith(query.toLowerCase());
      }, suggestionsAmount, submitOnSuggestionTap, clearOnSubmit, minLength, [], textCapitalization, decoration, style, keyboardType, textInputAction, controller, focusNode);
}
